...
function DrawArcadeScreen(){
    ...
    // set shader uniforms
    customMaterial.setFloat("curve",0.75);
    customMaterial.setFloat("scan",3-Math.cos(time)*2);
    customMaterial.setFloat("fisheye",0.3);
...
var createScene = function (engine, canvas) {
    ...
    randomColour()                                                                                          // set a random screen and light each time you load the page
    ...
    const pipeline = new BABYLON.DefaultRenderingPipeline("defaultPipeline", true, scene, [camera])         // default pipeline to use FXAA
    pipeline.samples = 4
...
function checkOrientation() {
    ...
    camera.position = new BABYLON.Vector3(camera.position.x,camera.position.y+2,camera.position.z-8);

    if(isMobile && window.innerWidth > window.innerHeight) {
        document.getElementById('PortraitFail').classList.remove("PortraitFail")
    } else if(isMobile && window.innerWidth < window.innerHeight) {
        document.getElementById('PortraitFail').classList.add("PortraitFail")
    }
}
